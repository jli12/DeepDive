...
def get_openclip_model(model_name):
    ...
    model, _, _ = open_clip.create_model_and_transforms(model=model_name, device='cpu') # 'ViT-B-32-quickgelu', pretrained='laion400m_e32')
    return model.visual
...
def get_openclip_transforms(model_name, input_type = 'PIL'):
    import clip; # _, preprocess = clip.load(model_name, device = 'cpu')
    _, _, preprocess = open_clip.create_model_and_transforms(model=model_name, device='cpu') # 'ViT-B-32-quickgelu', pretrained='laion400m_e32')
    if input_type == 'PIL':
        recommended_transforms = preprocess.transforms
    if input_type == 'numpy':
        recommended_transforms = [transforms.ToPILImage()] + preprocess.transforms
